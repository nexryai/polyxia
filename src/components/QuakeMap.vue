    ...
    const render = async () => {
        ...
        switch (quakeData.earthquake.domesticTsunami) {
            ...
            case "Warning":
                ...
                break;
            default:
                tsunamiStatusLabel.value = "津波情報の取得に失敗しました";
        ...
        if (quakeData.earthquake.foreignTsunami != "None" && quakeData.earthquake.foreignTsunami != "Unknown" && tsunamiStatus.value != TsunamiStatus.WARNING) {
            ...
            switch (quakeData.earthquake.foreignTsunami) {
                ...
                case "Potential":
                    ...
                    break;
                default:
                    tsunamiStatusLabel.value = "津波情報（海外）の取得に失敗しました";
                    tsunamiStatus.value = TsunamiStatus.NOTICE;
            }
        } else {
            if (isForeignQuake) {
        ...
        try {
            ...
            map.setView([quakeData.earthquake.hypocenter.latitude, quakeData.earthquake.hypocenter.longitude], 7);
            ...
            if (isForeignQuake) {
                ...
                // GeoJSON をマップに追加
                const geoJsonLayer = !mapInitialized
...
<template>
    <div class="debug-warning" v-if="isDebug"><span>DEBUG MODE - THIS IS DUMMY DATA</span></div>
    ...
    <div v-else>
        ...
        <div class="quake-info" v-if="!isLoading">
            <div class="tsunami-status" :style="tsunamiStatusStyle"><span class="tsunami-label">{{ tsunamiStatusLabel }}</span></div>
        </div>
    </div>
</template>
